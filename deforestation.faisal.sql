...
-- Checking if the view already exists and deleting it if it does
...
/*2. Regional Outlook*/ /*a. What was the percent forest of the entire world in 2016? Which region had the
...
SELECT
Round(SUM(forest_area_sq_km*100)/(SUM(land_area_sq_km)),2) AS percent_of_forest
...
/*3. COUNTRY-LEVEL DETAIL*/ /*a. Which 5 countries saw the largest amount decrease in forest area from 1990 to
 2016?
...
/*e. How many countries had a percent forestation higher than the United States in
2016?*/
SELECT COUNT(*) AS num_countries
FROM
  (SELECT per_forest_area_sqkm AS other
   FROM forestation
   WHERE YEAR = 2016
     AND (per_forest_area_sqkm IS NOT NULL)
     AND (country_name NOT LIKE 'United States')) t1
WHERE
    (SELECT per_forest_area_sqkm AS usa
     FROM forestation
     WHERE YEAR = 2016
       AND (per_forest_area_sqkm IS NOT NULL)
       AND (country_name LIKE 'United States')) < t1.other